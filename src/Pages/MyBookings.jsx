...
export default function MyBookings() {
  ...
  useEffect(() => {
    ...
    const getBookings = async () => {
      try {
        const res = await fetch(`https://hoteldripinn-qcajs976t-swatis-projects-d5718665.vercel.app/api/bookings/user/${user.id}`);
  ...
  const handleCancel = async (bookingId) => {
    ...
    try {
      const res = await fetch(`https://hoteldripinn-qcajs976t-swatis-projects-d5718665.vercel.app/api/bookings/${bookingId}/cancel`, {
        method: 'POST'
      });
      if (res.ok) {
